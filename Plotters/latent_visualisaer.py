...
import re
def load_raw_txt_smd_with_topology(machine_id, data_root, window_size=64):
    """
    Dynamically loads raw text data and applies the Consensus/Residual split
    based on the total sensor count of the specific machine.
    """
    print(f"📂 Loading raw text data for {machine_id}...")
    ...
    # --- DYNAMIC TOPOLOGY CALCULATION ---
    # We replicate the 63/37% split logic to match the saved weights.
    # For a 38-dim machine (like 2-1), this yields 24/14.
    # For a machine with different dims, it adjusts to match the trainer's logic.
    n_phy = int(total_dim * 0.6316) 
    n_res = total_dim - n_phy
    
    print(f"📊 Detected Topology: {n_phy} Consensus, {n_res} Residual (Total: {total_dim})")
    ...
    # Apply the dynamic split
    idx_phy = list(range(n_phy))
    idx_res = list(range(n_phy, total_dim))
    
    phy_data = X[:, :, idx_phy]
    res_data = X[:, :, idx_res]

    return {
        # Expand dims for 'phy' to simulate the [Anchor, Aug1...Aug4, Jerk] shape
        'phy': np.expand_dims(phy_data, axis=1), 
        'res': res_data,
        'labels': L,
        'dims': (n_phy, n_res) # Pass dims back to the main visualizer
    }
...
def visualize_dual_latent_space(config, test_final, weights_path, machine_name="Machine"):
    print(f"🚀 Initializing Visualization for {machine_name}...")
    
    W = config.get("window_size", 64)
    phy_dim = test_final['phy'].shape[-1]
    res_dim = test_final['res'].shape[-1]
    
    encoder = build_dual_encoder((W, phy_dim), (W, res_dim), config)
    
    print(f"📂 Loading weights from {weights_path}...")
    encoder.load_weights(os.path.join(weights_path, "encoder.weights.h5"))
    
    # Extract data
    phy_anchor = tf.cast(test_final['phy'][:, 0, :, :], tf.float32)
    res_windows = tf.cast(test_final['res'], tf.float32)
    window_labels = np.any(test_final['labels'] > 0, axis=1) # [cite: 617]

    # --- FIXED: Batched Inference to prevent GPU OOM ---
    print("🪄 Extracting latent features in batches...")
    batch_size = 128
    ...
    for i in range(0, len(phy_anchor), batch_size):
        ...
        # encoder returns [z_sys, z_res, z_combined]
    ...
    print("🪄 Computing t-SNE (sampling 5000 points for speed)...")
    tsne = TSNE(n_components=2, perplexity=30, random_state=42)
    
    # We only visualize a subset to keep the plot clean and t-SNE fast [cite: 615]
    indices = np.random.choice(len(z_sys), min(5000, len(z_sys)), replace=False)
    ...
    # Consensus Path (HNN) - 
    ax1.scatter(z_s_2d[~lbls, 0], z_s_2d[~lbls, 1], c='royalblue', label='Normal', alpha=0.5, s=10)
    ax1.scatter(z_s_2d[lbls, 0], z_s_2d[lbls, 1], c='crimson', label='Anomaly', marker='x', s=15)
    ax1.set_title(f"HNN Consensus Space - {machine_name}")

    # Residual Path (TCN) - [cite: 621, 633]
    ax2.scatter(z_r_2d[~lbls, 0], z_r_2d[~lbls, 1], c='seagreen', label='Normal', alpha=0.5, s=10)
    ax2.scatter(z_r_2d[lbls, 0], z_r_2d[lbls, 1], c='darkorange', label='Anomaly', marker='x', s=15)
    ax2.set_title(f"TCN Residual Space - {machine_name}")
    
    os.makedirs("results/plots", exist_ok=True)
    plt.savefig(f"results/plots/{machine_name}_tsne.png")
    plt.show()
...
if __name__ == "__main__":
    main_config = {
        ...
        "lambda_e": 1.0,
        "alpha_vpo": 2.0,
        "sentinel_weight": 5.0, # Weight for Dead Sentinel penalty
        "batch_size": 128,
        "patience": 5
    }
    
    target_machine = "machine-3-5"
    data_path = "/home/utsab/Downloads/smd/ServerMachineDataset"
    
    run_integrated_visualization(target_machine, data_path, main_config)