...
def load_msl_windows(data_root, machine_id, config):
    """
    Specialized loader for MSL (Mars Science Laboratory) telemetry.
    Processes 55 dimensions and parses labels from labelled_anomalies.csv.
    """
    ...
    # 1. Loading & Standardization [cite: 273]
    ...
    # 3. Envelopes & Jerk (Residual Branch)
    ...
    # 4. Windowing Utility [cite: 275, 276]
    ...
    # 5. Masking Views (ACAE Proxy Task) [cite: 26, 167, 300]
    from src.masking import get_masked_views, resi_masker
    ...
    test_final = {
        "phy": create_windows(test_phy, stride), # Use stride here, not window size
        "res": create_windows(test_res, stride), 
        "topology": topo
    }

    # 6. CSV Label Parsing (The Fix)
    ...
    # NASA column name is 'anomaly_sequences', not 'indices'
    ...
    # Length of original telemetry sequence captured by the windows
